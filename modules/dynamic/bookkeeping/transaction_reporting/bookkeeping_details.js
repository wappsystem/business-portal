...
//-------------------------------------
var param=_sys.config.parameters;
var predefined_item_tid=param.predefined_item_tid;
//-------------------------------------
$('#name__ID').autocomplete({
    minLength:0,
    source:function(request,response){
        var sql="with tb as (select name=S1 from [TABLE-"+predefined_item_tid+"])";
...
$('#type__ID').autocomplete({
    minLength:0,
    source:function(request,response){
        var sql="with tb as (select distinct name=S2 from [TABLE-"+predefined_item_tid+"])";
        sql+=" select top 10 name,value=name from tb where name like '%'+@S1+'%' ";
        $VmAPI.request({data:{cmd:'auto',s1:request.term,sql:sql,minLength:0},callback:function(res){
            response($vm.autocomplete_list(res.table));
        }});
